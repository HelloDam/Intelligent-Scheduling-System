...
import com.dam.model.dto.scheduling_calculate.EmployeePlan;
        ...
        for (int e : sequence) {
            for (int i = 0; i < shiftIndexList.get(e).size(); i++) {
                ...
                if (set.add(shiftIndex)) {
                    LogUtil.common("匹配-" + (removeIndexList.size() + 1) + ": 员工" + employees[e].getId() + " => 第" + (curDayIndex + 1) + "天的班次" + shiftList.get(shiftIndex).getKey());
                    ...
                    employeePlans[e].workTimeEachDay[curDayIndex] += (shiftList.get(shiftIndex).getLen() - (shiftList.get(shiftIndex).getMealType() == null ? 0 : shiftList.get(shiftIndex).getMealLen()));
